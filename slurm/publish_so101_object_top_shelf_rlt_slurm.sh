#!/bin/bash
#SBATCH --job-name=pi05_publish_object_top_shelf_rlt
#SBATCH --nodes=1
#SBATCH --gres=gpu:1
...
# pi05_rlt_so101_object_top_shelf — Stage-1 RL-token checkpoint publish to HF
...
#
# Cross-account note: training data may be owned by other HF accounts, but
# checkpoints upload to the `lorenzouttini` account below. These are independent —
# the write token just needs write access to YOUR (lorenzouttini) namespace; it
# has nothing to do with who owns the dataset.
#
# Cluster: Isambard u6kr. Submit from the worktree:
#   cd /home/u6kr/lorenzo.u6kr/openpi_so101_object_top_shelf_remote
#   sbatch slurm/publish_so101_object_top_shelf_rlt_slurm.sh
#
# Auto-run after training (uploads once training succeeds):
#   TRAIN_JOB=$(sbatch --parsable slurm/train_so101_object_top_shelf_rlt_slurm.sh)
#   sbatch --dependency=afterok:${TRAIN_JOB} slurm/publish_so101_object_top_shelf_rlt_slurm.sh
#
# Before sbatch:
...
HF_REPO_ID="lorenzouttini/pi05-rlt-so101-object-top-shelf-isambard"
