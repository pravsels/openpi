#!/bin/bash
#SBATCH --job-name=pi0_publish_armnetbench_tool_insert
#SBATCH --nodes=1
#SBATCH --gres=gpu:1
...
# pi0_armnetbench_tool_insert — raw checkpoint publish to Hugging Face.
#
...
#   sbatch slurm/publish_armnetbench_tool_insert_pi0_slurm.sh
#
# Auto-run after training (uploads once training succeeds):
#   TRAIN_JOB=$(sbatch --parsable slurm/train_armnetbench_tool_insert_pi0_slurm.sh)
#   sbatch --dependency=afterok:${TRAIN_JOB} slurm/publish_armnetbench_tool_insert_pi0_slurm.sh
#
# Before sbatch:
...
CONFIG_NAME="pi0_armnetbench_tool_insert"
EXP_NAME="armnetbench_tool_insert_pi0"
HF_REPO_ID="lorenzouttini/pi0-so101-armnetbench-tool-insert-isambard"
