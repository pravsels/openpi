#!/bin/bash
#SBATCH --job-name=pi05_rlt_so101_busybox_bimanual
#SBATCH --nodes=1
#SBATCH --gres=gpu:1
...
# pi05_rlt_so101_busybox_buttons_bimanual — Stage-1 RL-token training for the
# BIMANUAL SO101 busybox-buttons task (two arms, 12D dual-arm joint-space).
#
# This does NOT train a VLA. It attaches a learned RL-token encoder-decoder to
# the *frozen* baseline pi05 bimanual VLA (rl_vla_loss_weight=0.0,
...
#
# Dataset: pravsels/busybox_buttons_bimanual (cameras top/left_wrist/right_wrist).
# Base VLA: the fine-tuned isambard v50 checkpoint (NOT pi05_base) — see below.
#
...
#   sbatch slurm/train_so101_busybox_bimanual_rlt_slurm.sh
#
# Before first sbatch:
#   - openpi_arm64.sif at ${data_dir}/container/
#   - Baseline VLA at ${data_dir}/checkpoints/pi05_so101_busybox_buttons_bimanual_v50/step_24999/params
#       hf download lorenzouttini/pi05-so101-busybox-buttons-bimanual-isambard-v50 \
#         --local-dir ${data_dir}/checkpoints/pi05_so101_busybox_buttons_bimanual_v50
#     (the published repo nests weights under step_<N>/; we use the 25k-step
#      (step_24999) checkpoint. The RLT TrainConfig's weight_loader points at
#      .../pi05_so101_busybox_buttons_bimanual_v50/step_24999/params — keep in sync)
#   - HF / W&B tokens in ${scratch_dir}/.secrets/
#   - The pi05_rlt_so101_busybox_buttons_bimanual TrainConfig present in this
#     openpi checkout (git pull the branch that adds it before submitting).
...
# Published HF layout after
# `hf download lorenzouttini/pi05-so101-busybox-buttons-bimanual-isambard-v50 \
#    --local-dir .../pi05_so101_busybox_buttons_bimanual_v50`:
# the weights nest under step_<N>/params. We use the 25k-step (step_24999) checkpoint.
# Must match the RLT TrainConfig's weight_loader path.
BASE_VLA_PARAMS="${data_dir}/checkpoints/pi05_so101_busybox_buttons_bimanual_v50/step_24999/params"
...
if [ ! -d "${BASE_VLA_PARAMS}" ]; then
    ...
    echo "       Download the isambard v50 checkpoint first (see header)."
    exit 1
fi
