#!/bin/bash
#SBATCH --job-name=pi05_block_tower_joints_only
#SBATCH --nodes=1
#SBATCH --gres=gpu:4
...
# Joints-only ablation: trains only the first 7 (joint) dims of the 17D
# canonical block-tower action vector. EE channels of state are zeroed and
# the action-loss mask is forced to joints-only via `action_dim_mask`.
