#!/bin/bash
#SBATCH --job-name=pi05_arx5_micro_base
#SBATCH --nodes=1
#SBATCH --gres=gpu:4
...
# Train pi05_arx5_multitask_micro_baseline (14-dataset micro mix, 30k steps).
#
# Before sbatch, copy precomputed assets into ASSETS_DIR (must end with /assets):
#   training_mix_micro.json
#   norm_stats.json
#   valid_indices.json          (recommended; omit only if you want full index scan)
#
# Example from your laptop (adjust user, host, paths):
#   rsync -av ~/Downloads/pi05_arx5_multitask_micro_baseline/ \
#     pravsels.u6cr@<login>:/scratch/u6cr/pravsels.u6cr/openpi/checkpoints/pi05_arx5_multitask_micro_baseline/<exp_name>/assets/
...
CONFIG_NAME="pi05_arx5_multitask_micro_baseline"
...
if [ ! -f "${MIX_JSON}" ]; then
    ...
    echo "Rsync the mix JSON (and norm stats + valid indices) to ${ASSETS_DIR} before submitting."
    exit 1
fi
