#!/bin/bash
# GCloud Docker training script for RLT Stage 1 on busybox_multitask minmax (single-arm).
# Fresh a2-ultragpu-1g. Do not reuse openpi-rlt-busybox-multitask (relative 1%/99% run).
...
# the frozen pi05 busybox_multitask_minmax checkpoint (rl_vla_loss_weight=0.0) and
...
EXP_NAME="${EXP_NAME:-busybox_multitask_rlt_singlearm_minmax}"
...
BASE_VLA_DIR="${CHECKPOINT_DIR}/pi05_busybox_multitask_minmax"
BASE_VLA_PARAMS="${BASE_VLA_DIR}/params"
BASE_VLA_HF_REPO="pravsels/pi05_busybox_multitask_minmax"
MIN_FREE_GB="${MIN_FREE_GB:-200}"
EXPECTED_RCW_SHA="${EXPECTED_RCW_SHA:-597aa9ad21176e7f7dcee4aede5dc1ffc07eacee}"
EXPECTED_HUB_WANDB="${EXPECTED_HUB_WANDB:-swjv9hbs}"
...
if ! params_complete; then
    ...
    sudo docker run --rm \
      ...
      "${IMAGE}" \
      uv run python -c 'from huggingface_hub import snapshot_download; snapshot_download(repo_id="pravsels/pi05_busybox_multitask_minmax", local_dir="/workspace/repo/checkpoints/pi05_busybox_multitask_minmax")'
fi
if ! params_complete; then
