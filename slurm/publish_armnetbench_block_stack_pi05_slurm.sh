#!/bin/bash
#SBATCH --job-name=pi05_publish_armnetbench_block_stack
#SBATCH --nodes=1
#SBATCH --gres=gpu:1
...
# pi05_armnetbench_block_stack — raw checkpoint publish to Hugging Face.
#
...
#   sbatch slurm/publish_armnetbench_block_stack_pi05_slurm.sh
#
# Auto-run after training (uploads once training succeeds):
#   TRAIN_JOB=$(sbatch --parsable slurm/train_armnetbench_block_stack_pi05_slurm.sh)
#   sbatch --dependency=afterok:${TRAIN_JOB} slurm/publish_armnetbench_block_stack_pi05_slurm.sh
#
# Before sbatch:
...
CONFIG_NAME="pi05_armnetbench_block_stack"
EXP_NAME="armnetbench_block_stack_pi05"
HF_REPO_ID="lorenzouttini/pi05-so101-armnetbench-block-stack-isambard"
