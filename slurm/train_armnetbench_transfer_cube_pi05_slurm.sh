#!/bin/bash
#SBATCH --job-name=pi05_armnetbench_transfer_cube
#SBATCH --nodes=1
#SBATCH --gres=gpu:1
...
# pi05_armnetbench_transfer_cube — bimanual SO101 transferring a cube between the two arms (12D dual-arm joint-space, delta actions). Cameras top/left_wrist/right_wrist.
# Model: pi05 (NOT pi05.5). Dataset: villekuosmanen/armnetbench_transfer_cube (av1 + uniform
# resolution, 50 episodes, public, v3.0-tagged). Checkpoints publish to lorenzouttini.
...
#   sbatch slurm/train_armnetbench_transfer_cube_pi05_slurm.sh
#
# Before first sbatch:
