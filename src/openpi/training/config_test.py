...
def test_busybox_multitask_singlearm_rlt_config():
    three_cam = ("base_0_rgb", "left_wrist_0_rgb", "base_1_rgb")
    cfg = _config.get_config("pi05_rlt_busybox_multitask_singlearm")
    assert cfg.project_name == "busybox_multitask_rlt_singlearm"
    assert cfg.model.pi05 is True
    assert cfg.model.action_horizon == 30
    assert cfg.model.rl_vla_loss_weight == 0.0
    assert tuple(cfg.model.image_keys) == three_cam
    ...
    assert cfg.batch_size == 16
    assert cfg.fsdp_devices == 1
    assert cfg.weight_loader.params_path == "checkpoints/pi05_busybox_multitask/params"
    assert cfg.lr_schedule.warmup_steps == 1_000
    assert cfg.lr_schedule.peak_lr == 5e-5
    assert cfg.lr_schedule.decay_steps == 20_000
    assert cfg.lr_schedule.decay_lr == 5e-5
    ...
    bimanual = _config.get_config("pi05_rlt_busybox_multitask")
    assert isinstance(bimanual.data, _config.LeRobotSO101BimanualDataConfig)
    assert bimanual.num_train_steps == 10_000


def test_busybox_multitask_singlearm_rlt_gcloud_script_references_config():
    script = pathlib.Path("slurm/train_busybox_multitask_singlearm_rlt_gcloud.sh").read_text()
    assert 'CONFIG_NAME="pi05_rlt_busybox_multitask_singlearm"' in script
    assert "pravsels/pi05_busybox_multitask" in script
    assert "checkpoints/pi05_busybox_multitask/params" in script
    assert "WANDB_MODE=online" in script
    assert "80000" in script
    assert "MIN_FREE_GB" in script
    assert "refusing to recompute" in script
    ...
    assert '"${TRAIN_ARGS[@]}"' in script
    assert "${HOME}/.env" in script
    assert "pi05_rlt_busybox_multitask_singlearm" in script
    ...
    assert "4ym0qegc" in script
    helper = pathlib.Path("scripts/check_busybox_multitask_rcw_prompts.py").read_text()
    assert "rcw_sha_ok" in helper
    assert "prompt_ok" in helper
    assert "refusing to train on PyPI RCW" in helper
    assert "Move the left slider to position 1" in helper
...
def test_dockerfile_installs_rcw_from_git_main_not_pypi():
    ...
    assert "@main" in dockerfile or "rev=main" in dockerfile
    assert "RoboCandyWrapper" in dockerfile
...
def test_dockerfile_installs_rcw_from_git_main_not_pypi():
    ...
    assert "github.com/villekuosmanen/RoboCandyWrapper.git" in dockerfile
    assert "@main" in dockerfile or "rev=main" in dockerfile
    assert "robocandywrapper" in dockerfile.lower() or "RoboCandyWrapper" in dockerfile
