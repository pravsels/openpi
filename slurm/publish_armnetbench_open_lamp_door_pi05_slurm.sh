#!/bin/bash
#SBATCH --job-name=pi05_publish_armnetbench_open_lamp_door
#SBATCH --nodes=1
#SBATCH --gres=gpu:1
...
# pi05_armnetbench_open_lamp_door — raw checkpoint publish to Hugging Face.
#
...
#
# Cross-account note: training data is owned by the `villekuosmanen` HF account,
# but checkpoints upload to the `lorenzouttini` account below. These are
# independent — the write token just needs write access to YOUR (lorenzouttini)
# namespace; it has nothing to do with who owns the dataset.
#
# Cluster: Isambard u6kr. Submit from the worktree:
#   cd /home/u6kr/lorenzo.u6kr/openpi_so101_pi05_cable_shelf
#   sbatch slurm/publish_armnetbench_open_lamp_door_pi05_slurm.sh
#
# Auto-run after training (uploads once training succeeds):
#   TRAIN_JOB=$(sbatch --parsable slurm/train_armnetbench_open_lamp_door_pi05_slurm.sh)
#   sbatch --dependency=afterok:${TRAIN_JOB} slurm/publish_armnetbench_open_lamp_door_pi05_slurm.sh
#
# Before sbatch:
...
repo_dir="${home_dir}/openpi_so101_pi05_armnetbench"
...
CONFIG_NAME="pi05_armnetbench_open_lamp_door"
EXP_NAME="armnetbench_open_lamp_door_pi05"
HF_REPO_ID="lorenzouttini/pi05-so101-armnetbench-open-lamp-door-isambard"
