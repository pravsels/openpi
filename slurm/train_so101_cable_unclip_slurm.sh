#!/bin/bash
#SBATCH --job-name=pi05_so101_cable_unclip
#SBATCH --nodes=1
#SBATCH --gres=gpu:4
...
# pi05_so101_cable_unclip — SO101 unclipping cable from holder (6D joint-space, delta actions).
# Remote-teleop dataset villekuosmanen/cable_unclip_remote (public, apache-2.0).
# Already av1-encoded with correct resolutions (front/top 576x1024, wrist 720x1280) — no re-encode.
# Mirror to lorenzouttini for the v3.0 tag; checkpoints publish to lorenzouttini.
# The read token only needs read access to a public dataset.
#
...
#   sbatch slurm/train_so101_cable_unclip_slurm.sh
#
# Before first sbatch:
