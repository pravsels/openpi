#!/bin/bash
#SBATCH --job-name=pi05_publish_cable_unclip
#SBATCH --nodes=1
#SBATCH --gres=gpu:1
...
# pi05_so101_cable_unclip — raw checkpoint publish to Hugging Face.
#
...
# (mirrored to `lorenzouttini` for the v3.0 tag), but checkpoints upload to the
# `lorenzouttini` account below. These are independent — the write token just
# needs write access to YOUR (lorenzouttini) namespace.
#
# Cluster: Isambard u6kr. Submit from the worktree:
#   cd /home/u6kr/lorenzo.u6kr/openpi_so101_cable_unclip_remote
#   sbatch slurm/publish_so101_cable_unclip_slurm.sh
#
# Auto-run after training (uploads once training succeeds):
#   TRAIN_JOB=$(sbatch --parsable slurm/train_so101_cable_unclip_slurm.sh)
#   sbatch --dependency=afterok:${TRAIN_JOB} slurm/publish_so101_cable_unclip_slurm.sh
#
# Before sbatch:
...
repo_dir="${home_dir}/openpi_so101_cable_unclip_remote"
...
CONFIG_NAME="pi05_so101_cable_unclip"
EXP_NAME="so101_cable_unclip"
HF_REPO_ID="lorenzouttini/pi05-so101-cable-unclip-isambard"
