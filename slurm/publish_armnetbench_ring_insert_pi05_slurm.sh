#!/bin/bash
#SBATCH --job-name=pi05_publish_armnetbench_ring_insert
#SBATCH --nodes=1
#SBATCH --gres=gpu:1
...
# pi05_armnetbench_ring_insert — raw checkpoint publish to Hugging Face.
#
...
#   sbatch slurm/publish_armnetbench_ring_insert_pi05_slurm.sh
#
# Auto-run after training (uploads once training succeeds):
#   TRAIN_JOB=$(sbatch --parsable slurm/train_armnetbench_ring_insert_pi05_slurm.sh)
#   sbatch --dependency=afterok:${TRAIN_JOB} slurm/publish_armnetbench_ring_insert_pi05_slurm.sh
#
# Before sbatch:
...
CONFIG_NAME="pi05_armnetbench_ring_insert"
EXP_NAME="armnetbench_ring_insert_pi05"
HF_REPO_ID="lorenzouttini/pi05-so101-armnetbench-ring-insert-isambard"
