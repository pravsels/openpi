#!/bin/bash
#SBATCH --job-name=pi0_publish_armnetbench_block_stack
#SBATCH --nodes=1
#SBATCH --gres=gpu:1
...
# pi0_armnetbench_block_stack — raw checkpoint publish to Hugging Face.
#
...
#   sbatch slurm/publish_armnetbench_block_stack_pi0_slurm.sh
#
# Auto-run after training (uploads once training succeeds):
#   TRAIN_JOB=$(sbatch --parsable slurm/train_armnetbench_block_stack_pi0_slurm.sh)
#   sbatch --dependency=afterok:${TRAIN_JOB} slurm/publish_armnetbench_block_stack_pi0_slurm.sh
#
# Before sbatch:
...
CONFIG_NAME="pi0_armnetbench_block_stack"
EXP_NAME="armnetbench_block_stack_pi0"
HF_REPO_ID="lorenzouttini/pi0-so101-armnetbench-block-stack-isambard"
